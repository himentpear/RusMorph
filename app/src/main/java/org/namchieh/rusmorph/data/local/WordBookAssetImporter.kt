    ...
    suspend fun importIfNeeded() = withContext(Dispatchers.IO) {
        ...
        database.withTransaction {
            ...
            for (asset in catalog.books) {
                ...
                dao.clearDialogueLines(book.id)
                dao.clearTextParagraphs(book.id)
                dao.clearDialogues(book.id)
                dao.clearTexts(book.id)
                dao.clearLessonWords(book.id)
                dao.clearLessons(book.id)
