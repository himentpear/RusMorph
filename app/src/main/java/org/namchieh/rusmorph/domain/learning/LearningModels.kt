...
@Deprecated("Use WordBook")
typealias Course = WordBook

data class WordBookLesson(
    val id: String,
    ...
    val content: LessonContentAvailability = LessonContentAvailability(units.any { it.type == LearningUnitType.VOCABULARY }, units.any { it.type == LearningUnitType.DIALOGUE }, units.any { it.type == LearningUnitType.TEXT }),
    val progress: Float = 0f,
    val status: LearningStatus = LearningStatus.NOT_STARTED,
    val latestScore: Double? = null,
    val isReviewLesson: Boolean = false,
) {
    @Deprecated("Use wordBookId")
    val courseId: String get() = wordBookId
}
...
data class LearningProgress(val sourceId: String, val wordBookId: String?, val lessonId: String?, val unitType: LearningUnitType?, val progress: Float, val status: LearningStatus, val updatedAt: Long) { @Deprecated("Use wordBookId") val courseId: String? get() = wordBookId }
data class ReviewItem(val id: String, val type: ReviewItemType, val sourceId: String, val lessonId: String?, val dueAt: Long?, val interval: Int?, val difficulty: Double?, val mistakeCount: Int, val lastResult: Double?, val wordBookId: String? = null)
...
data class LearningActivity(val id: String, val type: LearningActivityType, val sourceId: String?, val wordBookId: String?, val lessonId: String?, val occurredAt: Long, val durationSeconds: Int = 0) { @Deprecated("Use wordBookId") val courseId: String? get() = wordBookId }
data class PronunciationSession(val id: String, val type: PronunciationSessionType, val sourceId: String?, val wordBookId: String?, val lessonId: String?, val targetText: String, val startedAt: Long, val completedAt: Long? = null, val intelligibilityScore: Double? = null) { @Deprecated("Use wordBookId") val courseId: String? get() = wordBookId }
...
data class AIContext(val type: AIContextType, val sourceId: String? = null, val wordBookId: String? = null, val lessonId: String? = null, val label: String? = null, val excerpt: String? = null) { @Deprecated("Use wordBookId") val courseId: String? get() = wordBookId }

@Deprecated("Use WordBookLesson")
typealias Lesson = WordBookLesson
data class LessonContentAvailability(val words: Boolean = false, val dialogues: Boolean = false, val texts: Boolean = false)
data class LessonWord(val wordBookId: String, val lessonId: String, val entryId: String, val order: Int, val isKey: Boolean, val entry: org.namchieh.rusmorph.data.local.LexiconEntryWithDetails)
fun scopedLearningId(wordBookId: String, lessonId: String): String = "${wordBookId.length}:$wordBookId$lessonId"

fun scopedContentId(wordBookId: String, lessonId: String, contentId: String): String = scopedLearningId(scopedLearningId(wordBookId, lessonId), contentId)
