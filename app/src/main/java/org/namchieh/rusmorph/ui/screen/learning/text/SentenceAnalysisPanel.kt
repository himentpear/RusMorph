...
fun SentenceAnalysisPanel(sentence: TextSentence, modifier: Modifier = Modifier) {
    Row(modifier.fillMaxWidth().height(IntrinsicSize.Min).padding(top = 8.dp)) {
        Box(Modifier.width(2.dp).fillMaxHeight().background(RusMorphColors.AccentOrange))
        Column(
            Modifier.fillMaxWidth().background(RusMorphColors.SurfaceMuted).padding(horizontal = 14.dp, vertical = 12.dp),
...
private fun AnalysisSection(title: String, values: List<String>, emptyText: String) {
    Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
        Text(title, style = RusMorphTechTypography.MicroPill, color = RusMorphColors.AccentOrange)
        if (values.isEmpty()) {
            Text(emptyText, style = androidx.compose.material3.MaterialTheme.typography.bodySmall, color = RusMorphColors.TextTertiary)
        } else {
            values.forEach { value ->
                Text(value, style = androidx.compose.material3.MaterialTheme.typography.bodySmall, color = RusMorphColors.TextSecondary)
            }
        }
    }
}
