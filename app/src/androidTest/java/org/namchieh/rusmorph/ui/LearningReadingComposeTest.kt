...
import org.namchieh.rusmorph.ui.screen.tools.ToolsScreen
...
class LearningReadingComposeTest {
    ...
    @Test fun homeShowsOnlyTheActiveCourseAndNoCourseSwitcher() {
        compose.setContent {
            RusMorphTheme {
                HomeScreen(
                    coursesState = Loadable.Content(courses),
                    stats = LearningStats(),
                    progress = emptyList(),
                    selectedCourseId = "book-1",
                    onCourse = {}, onContinue = { _, _ -> }, onDictionary = {}, onPronunciation = {},
                    onReview = {}, onBottom = {}, onAI = {},
                )
            }
        }
    ...
    @Test fun myTextbooksShowsBooksAndSelectsBeforeOpeningDirectory() {
        ...
        compose.setContent {
            RusMorphTheme {
                ToolsScreen(
                    ...
                    onPronunciation = {}, onAiCommands = {}, onNavigateToRules = {}, onSettings = {}, onBottom = {},
                )
            }
        }
    ...
    @Test fun sentenceAnalysisIsAccordionAndTranslationCanBeToggled() {
        ...
        compose.onNode(hasContentDescription("俄语句子：Каждый день мы учимся.") and hasStateDescription("已展开")).assertIsDisplayed()
        compose.onNode(hasContentDescription("俄语句子：Мы живём в Москве.") and hasStateDescription("已收起")).assertIsDisplayed()
        compose.onAllNodesWithText("暂无语法标注").assertCountEquals(1)

        compose.onNodeWithContentDescription("俄语句子：Каждый день мы учимся.").performClick()
        compose.onAllNodesWithText("暂无语法标注").assertCountEquals(0)
    }
}
