...
class AgentRepositoryTest {
    ...
    @Test fun parsesLegacyAskSuccess() = runTest {
        ...
        assertTrue(DefaultAgentRepository.create(server.url("/").toString()).ask(request) is AgentResult.Success)
    }
    ...
    @Test fun mapsNormalizedProviderErrors() = runTest {
        ...
        for ((status, pair) in cases) {
            ...
            val result = DefaultAgentRepository.create(server.url("/").toString()).routeCommand("слово", AgentSessionContext())
    ...
    @Test fun sendsTwoStageDtosAndMapsWordCard() = runTest {
        ...
        val repository = DefaultAgentRepository.create(server.url("/").toString())
