...
class WordBookRepositoryTest {
    ...
    @Test fun offlineCatalogImportsExactContentAndIsIdempotent() = runTest {
        ...
        val book = assets.wordBooks().single()
        val lessons = assets.lessons(book.id)
        assertEquals("brick", book.withProgress(lessons, emptyList()).visualIdentity)
        assertEquals("cover_university_russian_1", book.cover)
        assertEquals(18, lessons.size)
        assertEquals(1929, lessons.sumOf { assets.getLessonWords(book.id, it.id).size })
        assertEquals(17, lessons.sumOf { assets.lessonDialogues(book.id, it.id).size })
        assertTrue(lessons.all { assets.lessonTexts(book.id, it.id).isEmpty() && !it.content.texts })
        assertEquals(1929, lessons.sumOf { room.getLessonWords(book.id, it.id).size })
        WordBookAssetImporter(context, db).importIfNeeded()
        assertEquals(18, room.lessons(book.id).size)
        assertNull(assets.lesson("missing", lessons.first().id))
        assertTrue(assets.lessonWords(book.id, "fake-1").isEmpty())
        assertTrue(assets.lessonDialogues("missing", lessons.first().id).isEmpty())
    }
