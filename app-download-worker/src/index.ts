...
const APK_KEY = "werus-0.004.apk";
const LEGACY_APK_KEY = "werus-0.003.apk";
const VERSION = "0.004";
const VERSION_CODE = 4;
const DOWNLOAD_PATH = `/werus-latest.apk`;
const DOWNLOAD_NAME = `WeRus-${VERSION}.apk`;
const APK_SHA256 = "8df6c6b0c2f2a66c3984b095bd0c621e249c736f5bb08afb31b8dfdf85e62328";
...
async function stableUpdateManifest(env: Env, head: boolean): Promise<Response> {
  ...
  const manifest = {
    ...
    forceUpdate: false,
    publishedAt: "2026-09-22T10:29:40Z",
...
async function apkResponse(request: Request, env: Env): Promise<Response> {
  ...
  if (url.pathname.includes("0.003")) {
    keyToUse = "werus-0.003.apk";
  } else if (url.pathname.includes("0.3.0")) {
    keyToUse = "werus-0.3.0.apk";
  } else if (url.pathname.startsWith("/rusmorph")) {
    keyToUse = "rusmorph-0.004.apk";
  } else {
    keyToUse = APK_KEY;
  }
  let metadata = await env.DOWNLOADS.head(keyToUse);
  if (!metadata && keyToUse !== APK_KEY) {
    keyToUse = APK_KEY;
    metadata = await env.DOWNLOADS.head(keyToUse);
  }
  if (!metadata) {
